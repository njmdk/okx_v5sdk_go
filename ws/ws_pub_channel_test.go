...
func TestKLine(t *testing.T) {
	...
	arg["instId"] = "BTC-USDT"
	// 1分钟K
	arg["channel"] = "candle" + string(wImpl.PERIOD_1MIN)
	args = append(args, arg)

	start := time.Now()
	res, _, err = r.PubKLine(OP_SUBSCRIBE, args)
	if res {
		usedTime := time.Since(start)
		fmt.Println("订阅成功！", usedTime.String())
	} else {
	...
	start = time.Now()
	res, _, err = r.PubKLine(OP_UNSUBSCRIBE, args)
	if res {
		usedTime := time.Since(start)
		fmt.Println("取消订阅成功！", usedTime.String())
	} else {
...
func TestKLineIndex(t *testing.T) {
	...
	start := time.Now()
	res, _, err = r.PubKLineIndex(OP_SUBSCRIBE, args)
	if res {
		usedTime := time.Since(start)
		fmt.Println("订阅成功！", usedTime.String())
	} else {
	...
	start = time.Now()
	res, _, err = r.PubKLineIndex(OP_UNSUBSCRIBE, args)
	if res {
		usedTime := time.Since(start)
		fmt.Println("取消订阅成功！", usedTime.String())
	} else {
