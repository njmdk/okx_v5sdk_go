...
import (
	...
	"errors"
	"fmt"
	"github.com/njmdk/okx_v5sdk_go/config"
	"github.com/njmdk/okx_v5sdk_go/utils"
...
// 普通订阅推送数据回调函数
type ReceivedMsgDataCallback func(time.Time, wImpl.MsgData) error

// 深度订阅推送数据回调函数
type ReceivedDepthDataCallback func(time.Time, wImpl.DepthData) error

// websocket
type WsClient struct {
	WsEndPoint string
	WsApi      *config.ApiInfo
	conn       *websocket.Conn
	sendCh     chan string //发消息队列
	resCh      chan *Msg   //收消息队列

	errCh chan *Msg
	regCh map[wImpl.Event]chan *Msg //请求响应队列
	...
	// 记录深度信息
	DepthDataList map[string]wImpl.DepthDetail
...
// 创建ws对象
func NewWsClient(ep string, debug bool) (r *WsClient, err error) {
	if ep == "" {
	...
	r = &WsClient{
		...
		resCh:      make(chan *Msg),
		errCh:      make(chan *Msg),
		regCh:      make(map[wImpl.Event]chan *Msg),
		//cbs:        make(map[Event]ReceivedDataCallback),
		quitCh:        make(chan struct{}),
		DepthDataList: make(map[string]wImpl.DepthDetail),
...
新增记录深度信息
*/
func (a *WsClient) addDepthDataList(key string, dd wImpl.DepthDetail) error {
...
*/
func (a *WsClient) updateDepthDataList(key string, dd wImpl.DepthDetail) error {
	a.DepthDataLock.Lock()
	defer a.DepthDataLock.Unlock()
...
获取当前的深度快照信息(合并后的)
*/
func (a *WsClient) GetSnapshotByChannel(data wImpl.DepthData) (snapshot *wImpl.DepthDetail, err error) {
	...
	snapshot = new(wImpl.DepthDetail)
...
func (a *WsClient) work() {
	...
	for {
		...
		case <-ticker.C: // 保持心跳
			// go a.Ping(1000)
			if time.Now().Unix()-a.lastTime > 30 {
				a.lastTime = time.Now().Unix()
				go func() {
					_, _, err := a.Ping(1000)
					if err != nil {
...
func (a *WsClient) receive() {
	...
	for {
		...
		if !ok {
			//只有推送消息才会主动创建通道和消费队列
			if evt == wImpl.EVENT_BOOKED_DATA || evt == wImpl.EVENT_DEPTH_DATA {
				...
				go func(evt wImpl.Event) {
					...
					for msg := range a.regCh[evt] {
						...
						switch evt {
						// 处理普通推送数据
						case wImpl.EVENT_BOOKED_DATA:
							fn := a.onBookMsgHook
							if fn != nil {
								err = fn(msg.Timestamp, msg.Info.(wImpl.MsgData))
								if err != nil {
						...
						// 处理深度推送数据
						case wImpl.EVENT_DEPTH_DATA:
							fn := a.onDepthHook

							depData := msg.Info.(wImpl.DepthData)
							...
							// 运行用户定义回调函数
							if fn != nil {
								err = fn(msg.Timestamp, msg.Info.(wImpl.DepthData))
								if err != nil {
...
*/
func (a *WsClient) MergeDepth(depData wImpl.DepthData) (err error) {
	if !a.autoDepthMgr {
		return
	}
	...
	if depData.Action == "snapshot" {
		...
		var newSnapshot *wImpl.DepthDetail
...
通过ErrorCode判断事件类型
*/
func GetInfoFromErrCode(data wImpl.ErrData) wImpl.Event {
	switch data.Code {
	case "60001":
		return wImpl.EVENT_LOGIN
	case "60002":
		return wImpl.EVENT_LOGIN
	case "60003":
		return wImpl.EVENT_LOGIN
...
解析消息类型
*/
func (a *WsClient) parseMessage(raw []byte) (evt wImpl.Event, data interface{}, err error) {
	evt = wImpl.EVENT_UNKNOWN
	//log.Println("解析消息")
	//log.Println("消息内容:", string(raw))
	if string(raw) == "pong" {
		evt = wImpl.EVENT_PING
		data = raw
		return
	}
	//log.Println(0, evt)
	var rspData = wImpl.RspData{}
	err = json.Unmarshal(raw, &rspData)
	if err == nil {
		op := rspData.Event
		if op == OP_SUBSCRIBE || op == OP_UNSUBSCRIBE {
			channel := rspData.Arg["channel"]
			evt = wImpl.GetEventId(channel)
			data = rspData
			return
		}
	}
	...
	//log.Println("ErrData")
	var errData = wImpl.ErrData{}
	err = json.Unmarshal(raw, &errData)
	if err == nil {
		op := errData.Event
		switch op {
		case OP_LOGIN:
			evt = wImpl.EVENT_LOGIN
			data = errData
			//log.Println(3, evt)
			return
		case OP_ERROR:
			...
			if evt != wImpl.EVENT_UNKNOWN {
				return
			}
			evt = wImpl.GetEventId(GetInfoFromErrMsg(errData.Msg))
			if evt == wImpl.EVENT_UNKNOWN {
				evt = wImpl.EVENT_ERROR
				return
			}
			return
		}
	...
	//log.Println("JRPCRsp")
	var jRPCRsp = wImpl.JRPCRsp{}
	err = json.Unmarshal(raw, &jRPCRsp)
	if err == nil {
		data = jRPCRsp
		evt = wImpl.GetEventId(jRPCRsp.Op)
		if evt != wImpl.EVENT_UNKNOWN {
			return
		}
	}

	var depthData = wImpl.DepthData{}
	err = json.Unmarshal(raw, &depthData)
	if err == nil {
		evt = wImpl.EVENT_DEPTH_DATA
		data = depthData
	...
	//log.Println("MsgData")
	var msgData = wImpl.MsgData{}
	err = json.Unmarshal(raw, &msgData)
	if err == nil {
		evt = wImpl.EVENT_BOOKED_DATA
		data = msgData
