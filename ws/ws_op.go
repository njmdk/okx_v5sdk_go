...
import (
	...
	"github.com/njmdk/okx_v5sdk_go/rest"
	"github.com/njmdk/okx_v5sdk_go/utils"
	"github.com/njmdk/okx_v5sdk_go/ws/wImpl"
	"github.com/njmdk/okx_v5sdk_go/ws/wInterface"
	"log"
	"sync"
	"time"
)
...
func (a *WsClient) Login(apiKey, secKey, passPhrase string, timeOut ...int) (res bool, detail *ProcessDetail, err error) {
	...
	info, _ := msg[0].Info.(wImpl.ErrData)
...
等待结果响应
*/
func (a *WsClient) waitForResult(e wImpl.Event, timeOut int) (data interface{}, err error) {
...
发送消息到服务端
*/
func (a *WsClient) Send(ctx context.Context, op wInterface.WSReqData) (err error) {
	select {
	case <-ctx.Done():
		log.Println("发生失败退出！")
...
func (a *WsClient) process(ctx context.Context, e wImpl.Event, op wInterface.WSReqData) (data []*Msg, err error) {
	defer func() {
		_ = recover()
	}()
...
*/
func GetEventByParam(param map[string]string) (evtId wImpl.Event) {
	evtId = wImpl.EVENT_UNKNOWN
	channel, ok := param["channel"]
	if !ok {
		return
	}

	evtId = wImpl.GetEventId(channel)
	return
}
...
func (a *WsClient) PubChannel(evtId wImpl.Event, op string, params []map[string]string, timeOut ...int) (res bool, msg []*Msg, err error) {
...
// 参数校验
func checkParams(evtId wImpl.Event, params []map[string]string, pd wImpl.Period) (res []map[string]string, err error) {
