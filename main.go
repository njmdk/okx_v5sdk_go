...
import (
	...
	"github.com/njmdk/okx_v5sdk_go/ws"
	"log"
	"time"
)
...
func REST() {
	...
	cli := rest.NewRESTClient("https://www.okx.com", &apikey, true, true)
...
func wsPriv() {
	...
	start := time.Now()
	res, _, err = r.PrivAccout(ws.OP_SUBSCRIBE, args)
	if res {
	...
	start = time.Now()
	res, _, err = r.PrivAccout(ws.OP_UNSUBSCRIBE, args)
	if res {
		usedTime := time.Since(start)
