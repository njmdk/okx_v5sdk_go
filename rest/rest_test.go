...
func TestRESTAPIGet(t *testing.T) {
	...
	rest := NewRESTAPI("https://www.okx.com", GET, "/api/v5/account/balance", nil, true)
	...
	cli := NewRESTClient("https://www.okx.com", &apikey, true, true)
...
func TestRESTAPIPost(t *testing.T) {
	...
	rest := NewRESTAPI("https://www.okx.com", POST, "/api/v5/account/set-greeks", &param, true)
	rest.SetSimulate(true).SetAPIKey("xxxx", "xxxx", "xxxx")
	response, err := rest.Run(context.Background())
	if err != nil {
		fmt.Println(err)
